...
public class DBLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        taskRepository.save(new Task("Adapter pattern", "Create adapter which implements Costable (calcCost) for Priceble (getPrice)", TaskStatus.NEW));
        taskRepository.save(new Task("Builder pattern", "Create immutable object (Builder pattern)", TaskStatus.NEW));
        taskRepository.save(new Task("Template method pattern", "Implement Template method pattern", TaskStatus.NEW));
